...
class ParseFitMessages(object):
    """docstring for ParseFitMessages"""
    def __init__(self, ):
        super(ParseFitMessages, self).__init__()
        self.fh = self.open_report('build_app/stx2_tenrec_fpga.fit.rpt')
        self.parse()
    ...
    def parse(self, ):
        watch = False
        html = self.open_html("html/views/message_fit.html")
        header = '<div class="container-fluid">'
        header += '<div class="row" style="padding:50px">'
        header += '<h3>Fit messages</h3><p>'
        ...
        for idx, line in enumerate(self.fh.readlines()):
            ...
            if line.startswith("; Fitter Messages"):
                watch = True
            elif line.startswith("; Fitter Suppressed Messages"):
                break
            elif watch == True:
