# UART register accessor.
# Generated with the command:
# python gen_uart_regs.py LEDR=4:out:unsigned ctrl_ping_rate=26:out ctrl_pulse_length=23:out ctrl_pwm_width=13:out ctrl_custom_delay=20:out:unsigned ctrl_start_slope=12:out ctrl_slope_rate=20:out ctrl_trap_slope=32:out
...
registers = [
    ('LEDR', 'out', 4, 'unsigned'),
    ('ctrl_ping_rate', 'out', 26, 'std_logic_vector'),
    ('ctrl_pulse_length', 'out', 23, 'std_logic_vector'),
    ('ctrl_pwm_width', 'out', 13, 'std_logic_vector'),
    ('ctrl_custom_delay', 'out', 20, 'unsigned'),
    ('ctrl_start_slope', 'out', 12, 'std_logic_vector'),
    ('ctrl_slope_rate', 'out', 20, 'std_logic_vector'),
    ('ctrl_trap_slope', 'out', 32, 'std_logic_vector'),
]
...
class UartRegs:
    ...
    def print_byte_debug(self, byte):
        protocol_constants = {
            ...
            UART_START_WRITE: "UART_START_WRITE",
            UART_END_WRITE: "UART_END_WRITE",
    ...
    def read_regs(self):
        ...
        if self.debug:
            ...
            self.print_byte_debug(UART_READ_REQ)
        self.ser.write(bytes([UART_READ_REQ]))
        ...
        while True:
            ...
            if byte == UART_START_WRITE and prev_byte != UART_ESCAPE:
                receiving_data = True
            elif receiving_data:
                if byte == UART_END_WRITE and not escape_next:
                    receiving_data = False
                    break
                elif byte != UART_ESCAPE or escape_next:
                    data.append(byte)
                    escape_next = False
    ...
    def write_regs(self, write_values):
        ...
        write_bytes.append(UART_START_WRITE) # Directly append the integer

        for i in range(0, len(write_data_bits), 8):
            byte_value = int(write_data_bits[i:i + 8], 2)
            if byte_value in [UART_READ_REQ, UART_START_WRITE, UART_END_WRITE, UART_ESCAPE]: # Directly use the integers
                write_bytes.append(UART_ESCAPE) # Directly append the integer
            write_bytes.append(byte_value)
        write_bytes.append(UART_END_WRITE) # Directly append the integer
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Command-line interface to read from and write to UART-accessible registers.",
